...
namespace RCA_StudyManagementSystem.Client.Pages.Exports
{
    public partial class List : Microsoft.AspNetCore.Components.ComponentBase
    {
        ...
        protected bool ShowEligibility = false;
        ...
        private async Task SetPathReports()
        {
            ...
            PathReports = await PathReportData.ListPathReportsByStudyForExportAsync(StudyId);
            //await SetPathId(); // Also sets StudyName
            ...
            {
                //item.StudyColor = item.PathReports.FirstOrDefault().StudyColor;
        ...
        private async Task OnStudySelectChanged(Study value)
        {
            ...
            await StudyColorChanged.InvokeAsync(StudyColor);

            await SetPathReports();

            ShowEligibility = false;
            _eligItems = new List<PathReportView>();
            PathReports = new List<PathReportView>();
        ...
        private async Task RecheckEligibility(Guid studyId)
        {
            ...
            foreach (var pathView in PathReports)
            {
                var patient = await PatientData.GetPatientAsync(pathView.PatientId);
                var pathReport = patient.PathReports.FirstOrDefault(pr => pr.PathReportId == pathView.PathReportId);
                var pathReportView = pathView;

                if (pathReport != null)
                {
                    Study.CheckEligibility(patient, pathReport, ref pathReportView);
                    pathReportView.IsCaseMatchValid = !await CheckCaseMatches.DoesCaseMatchAsync(pathView.LastName, pathView.FirstName, patient.DateOfBirth, pathView.SocialSecurityNumber);
                    pathReportView.IsDNCValid = !await CheckDNC.DoesDNCMatchAsync(pathView.LastName, pathView.FirstName, patient.DateOfBirth, pathView.SocialSecurityNumber);
                    reCheckedPathReports.Add(pathReportView);
                }
            }
            _eligItems = reCheckedPathReports;
        ...
        private async Task OnExport(Guid StudyId)
        {
            ...
            var newBatch = new Batch
            {
                ...
                StudyId = StudyId,
                NumberOfCases = _displayItems.Count().ToString(),
            ...
            var exportData = await PathReportData.ExportPathReportDataAsync(Study.StudyId, "Current", Guid.Empty, pathIds, isReport);
        ...
        private async Task OnGenerateReport(Guid StudyId)
        {
            ...
            {
                IsExported = false;
                Snackbar.Add("No data available for report.", Severity.Warning);
            }
        }
