...
namespace RCA_StudyManagementSystem.Client.Pages.ReimbursementEntities
{
    public partial class List : Microsoft.AspNetCore.Components.ComponentBase
    {
        ...
        MudDataGrid<ReimbursementEntity>? reimbGrid { get; set; }

        public CancellationToken CancellationToken { get; set; } = new CancellationToken();

        public IEnumerable<ReimbursementEntity>? ReimbursementEntities { get; set; }

        private IEnumerable<ReimbursementEntity>? _displayItems { get; set; }

        private int Index = 0;



        private const string GridStateStorageKey = "ReimbDataGridState"; // Key for local storage
        ...
        private Func<ReimbursementEntity, bool> _quickFilter => x =>
        {
            ...
            if (x.Name!.Contains(_searchString, StringComparison.OrdinalIgnoreCase))
                return true;
            if (x.PayableTo!.Contains(_searchString, StringComparison.OrdinalIgnoreCase))
                return true;

            if (x.AttentionTo!.Contains(_searchString, StringComparison.OrdinalIgnoreCase))
                return true;
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            ReimbursementEntities = await ReimbursementEntityData.ListReimbursementEntitiesAsync(CancellationToken);

            _displayItems = ReimbursementEntities;




            reimbGrid.FilterDefinitions = ReimbGridStateView.FilterDefinitions;
            reimbGrid.SortDefinitions = new Dictionary<string, SortDefinition<ReimbursementEntity>>(ReimbGridStateView.SortDefinitions);

            reimbGrid.CurrentPage = ReimbGridStateView.CurrentPage;
                ...
                if (storedStateDto != null)
                {
                    ...
                    var newFilterDefs = storedStateDto.Filters
                        .Select(dto =>
                        {
                            ...
                            var column = reimbGrid.RenderedColumns.FirstOrDefault(c => c.PropertyName == dto.Field);
                            if (column == null)
                                return null;
                    ...
                    var newSortDefs = storedStateDto.Sorts
                        .ToDictionary(dto => dto.SortBy, dto => new SortDefinition<ReimbursementEntity>(
                            dto.SortBy,
                            dto.Descending,
                            0,
                            null
                    ...
                    var newSortColumn = ReimbGridStateView.SortColumn;

                    ReimbGridStateView.FilterDefinitions = newFilterDefs!;
                    ReimbGridStateView.SortDefinitions = newSortDefs;

                    reimbGrid.FilterDefinitions = ReimbGridStateView.FilterDefinitions;
                    reimbGrid.SortDefinitions = ReimbGridStateView.SortDefinitions;

                    // Set the sort definitions
                    if (ReimbGridStateView.SortDefinitions.Any())
                    {
                        // Apply the first sort definition if available
                        var firstSort = ReimbGridStateView.SortDefinitions.First();
                        SortDirection direction = firstSort.Value.Descending ? SortDirection.Descending : SortDirection.Ascending;
                        await reimbGrid.SetSortAsync(firstSort.Key, direction, firstSort.Value.SortFunc);

                        var sortDefinitions = reimbGrid.SortDefinitions; // Get current sort settings

                        string sortByProperty = "";
                        var sortedData = ReimbursementEntities.AsQueryable(); // Start with unsorted data



                        if (sortDefinitions != null && sortDefinitions.Any())
                        {
                            sortedData = ReimbursementEntities.AsQueryable(); // Start with unsorted data
                        ...
                        {
                            _displayItems = ReimbursementEntities; // If no sorting, revert to original order
                        }
                    }
                    // Load CurrentPage and PageSize
                    // Check that the dataGrid reference is not null

                    ReimbGridStateView.CurrentPage = storedStateDto.CurrentPage;
        ...
        {
            ReimbGridStateView.CurrentPage = page;
        ...
        async Task<IDialogReference> ViewItem(ReimbursementEntity args, ReimbursementEntity pArgs)
        {

            var newReimbursementEntities = new List<ReimbursementEntity>();

            foreach (var reimb in reimbGrid.FilteredItems)
            {
                var newReimbursementEntity = await ReimbursementEntityData.GetReimbursementEntityAsync(reimb.ReimbursementEntityId);
                newReimbursementEntities.Add(newReimbursementEntity);
            }
            ...
            parameters.Add(p => p.CarouselRecords, newReimbursementEntities); // Pass filtered items
            parameters.Add(p => p.InitialSelectedIndex, reimbGrid.FilteredItems.ToList().IndexOf(pArgs)); // Set initial position

            var options = _options;

            return await DialogService.ShowAsync<ViewDialog>("Reimbursement Entity View", parameters, options);
        ...
        public async void Dispose()
        {
            ...
            if (reimbGrid != null)
            {
                ...
                var filtersToSave = reimbGrid.FilterDefinitions
                ...
                var sortsToSave = reimbGrid.SortDefinitions.Values // Iterate over values of the Dictionary
                ...
                {
                    var sortColum = reimbGrid.RenderedColumns.FirstOrDefault(c => c.PropertyName == sortsToSave.FirstOrDefault()!.SortBy)?.Title ?? string.Empty;
                    ReimbGridStateView.SortColumn = sortColum;
                }
                ...
                var stateDto = new GridStateDto
                {
                    ...
                    Sorts = sortsToSave,
                    CurrentPage = reimbGrid.CurrentPage, // Save current page
                ...
                ReimbGridStateView.SortDefinitions = stateDto.Sorts
                    .ToDictionary(s => s.SortBy, s => new SortDefinition<ReimbursementEntity>(
                        s.SortBy,
                        s.Descending,
                        s.Index,
                        (Func<ReimbursementEntity, object>)s.SortFunc
                    ));



                Console.WriteLine($"State saved to LocalStorage. Filters Count: {filtersToSave.Count}, Sorts Count: {sortsToSave.Count}, Page: {reimbGrid.CurrentPage}");
            }
            else
            {
