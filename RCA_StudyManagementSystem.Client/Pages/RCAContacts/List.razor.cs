...
namespace RCA_StudyManagementSystem.Client.Pages.RCAContacts
{
    public partial class List : Microsoft.AspNetCore.Components.ComponentBase
    {
        ...
        MudDataGrid<RCAContact>? rcacGrid { get; set; }

        public CancellationToken CancellationToken { get; set; } = new CancellationToken();

        public IEnumerable<RCAContact>? RCAContacts { get; set; }

        private IEnumerable<RCAContact>? _displayItems { get; set; }

        private int Index = 0;



        private const string GridStateStorageKey = "RcaCDataGridState"; // Key for local storage
        ...
        private Func<RCAContact, bool> _quickFilter => x =>
        {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            RCAContacts = await RCAContactData.ListRCAContactsAsync(CancellationToken);

            _displayItems = RCAContacts;




            rcacGrid.FilterDefinitions = RcaCGridStateView.FilterDefinitions;
            rcacGrid.SortDefinitions = new Dictionary<string, SortDefinition<RCAContact>>(RcaCGridStateView.SortDefinitions);

            rcacGrid.CurrentPage = RcaCGridStateView.CurrentPage;
                ...
                if (storedStateDto != null)
                {
                    ...
                    var newFilterDefs = storedStateDto.Filters
                        .Select(dto =>
                        {
                            ...
                            var column = rcacGrid.RenderedColumns.FirstOrDefault(c => c.PropertyName == dto.Field);
                            if (column == null)
                                return null;
                    ...
                    var newSortDefs = storedStateDto.Sorts
                        .ToDictionary(dto => dto.SortBy, dto => new SortDefinition<RCAContact>(
                            dto.SortBy,
                            dto.Descending,
                            0,
                            null
                    ...
                    var newSortColumn = RcaCGridStateView.SortColumn;

                    RcaCGridStateView.FilterDefinitions = newFilterDefs!;
                    RcaCGridStateView.SortDefinitions = newSortDefs;

                    rcacGrid.FilterDefinitions = RcaCGridStateView.FilterDefinitions;
                    rcacGrid.SortDefinitions = RcaCGridStateView.SortDefinitions;

                    // Set the sort definitions
                    if (RcaCGridStateView.SortDefinitions.Any())
                    {
                        // Apply the first sort definition if available
                        var firstSort = RcaCGridStateView.SortDefinitions.First();
                        SortDirection direction = firstSort.Value.Descending ? SortDirection.Descending : SortDirection.Ascending;
                        await rcacGrid.SetSortAsync(firstSort.Key, direction, firstSort.Value.SortFunc);

                        var sortDefinitions = rcacGrid.SortDefinitions; // Get current sort settings

                        string sortByProperty = "";
                        var sortedData = RCAContacts.AsQueryable(); // Start with unsorted data



                        if (sortDefinitions != null && sortDefinitions.Any())
                        {
                            sortedData = RCAContacts.AsQueryable(); // Start with unsorted data
                        ...
                        {
                            _displayItems = RCAContacts; // If no sorting, revert to original order
                        }
                    }
                    // Load CurrentPage and PageSize
                    // Check that the dataGrid reference is not null

                    RcaCGridStateView.CurrentPage = storedStateDto.CurrentPage;
        ...
        {
            RcaCGridStateView.CurrentPage = page;
        ...
        async Task<IDialogReference> ViewItem(RCAContact args, RCAContact pArgs)
        {

            var newRCAContacts = new List<RCAContact>();

            foreach (var rcac in rcacGrid.FilteredItems)
            {
                var newRCAContact = await RCAContactData.GetRCAContactAsync(rcac.RCAContactId);
                newRCAContacts.Add(newRCAContact);
            }
            ...
            parameters.Add(p => p.CarouselRecords, newRCAContacts); // Pass filtered items
            parameters.Add(p => p.InitialSelectedIndex, rcacGrid.FilteredItems.ToList().IndexOf(pArgs)); // Set initial position

            var options = _options;

            return await DialogService.ShowAsync<ViewDialog>("RCA Contact View", parameters, options);
        ...
        public async void Dispose()
        {
            ...
            if (rcacGrid != null)
            {
                ...
                var filtersToSave = rcacGrid.FilterDefinitions
                ...
                var sortsToSave = rcacGrid.SortDefinitions.Values // Iterate over values of the Dictionary
                ...
                {
                    var sortColum = rcacGrid.RenderedColumns.FirstOrDefault(c => c.PropertyName == sortsToSave.FirstOrDefault()!.SortBy)?.Title ?? string.Empty;
                    RcaCGridStateView.SortColumn = sortColum;
                }
                ...
                var stateDto = new GridStateDto
                {
                    ...
                    Sorts = sortsToSave,
                    CurrentPage = rcacGrid.CurrentPage, // Save current page
                ...
                RcaCGridStateView.SortDefinitions = stateDto.Sorts
                    .ToDictionary(s => s.SortBy, s => new SortDefinition<RCAContact>(
                        s.SortBy,
                        s.Descending,
                        s.Index,
                        (Func<RCAContact, object>)s.SortFunc
                    ));



                Console.WriteLine($"State saved to LocalStorage. Filters Count: {filtersToSave.Count}, Sorts Count: {sortsToSave.Count}, Page: {rcacGrid.CurrentPage}");
            }
            else
            {
